...
export const DiffStyle = Schema.Literals(["auto", "stacked"]).annotate({
  description: "控制差异渲染样式：auto 自适应终端宽度，stacked 始终单列显示",
})
...
}).annotate({ description: "注意力通知和声音设置" })

const PromptSize = Schema.Int.check(Schema.isGreaterThan(0))
export const Prompt = Schema.Struct({
  max_height: Schema.optional(PromptSize).annotate({ description: "提示文本区域最大高度" }),
  max_width: Schema.optional(Schema.Union([PromptSize, Schema.Literal("auto")])).annotate({
    description: "首页提示最大宽度：正整数表示固定上限，或 auto 随终端宽度自适应",
  }),
}).annotate({ description: "提示尺寸设置" })
