...
function DiffViewer(props: { api: TuiPluginApi }) {
  ...
  useBindings(() => ({
    ...
    bindings: [
      ...
      { key: "pagedown,ctrl+f", cmd: "diff.page.down", desc: "向下翻页差异查看器" },
      { key: "pageup,ctrl+b", cmd: "diff.page.up", desc: "向上翻页差异查看器" },
      { key: "m", cmd: "diff.mark_reviewed", desc: "标记选中文件已审查" },
      ...
      <PanelGroup axis="y" width="100%" height="100%">
          ...
          <Switch>
            <Match when={diff.loading}>
              ...
              <box flexGrow={1} paddingLeft={1}>
                <text fg={theme().textMuted}>加载差异中...</text>
              </box>
            </Match>
        ...
        <Panel flexShrink={0} gap={2} paddingLeft={1} border="none">
          ...
          <Show when={nextHunkShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>下一块</span>
              </text>
            )}
          </Show>
          <Show when={previousHunkShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>上一块</span>
              </text>
            )}
          </Show>
          ...
          <Show when={markReviewedShortcut()}>
            {(shortcut) => (
              <text fg={theme().text}>
                {shortcut()} <span style={{ fg: theme().textMuted }}>标记已审查</span>
              </text>
            )}
          </Show>
